...
export async function POST(
  ...
  try {
    ...
    console.log('🧠 Proxying admin analyze to backend for ID:', id)
    console.log('🔗 Backend URL (redirected):', `${BACKEND_URL}/api/admin/analyze/${id}`)
    ...
    // Redirect to the correct backend endpoint: /api/admin/analyze/{id}
    const response = await fetch(`${BACKEND_URL}/api/admin/analyze/${id}`, {
      method: 'POST',
      headers: {
    ...
    console.error('❌ Admin analyze proxy error:', error)
    return NextResponse.json(
      { 
        success: false, 
