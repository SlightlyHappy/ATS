...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📤 Proxying admin resume upload to backend...')
    console.log('📋 Form data fields:', Array.from(formData.keys()))

    const response = await fetch(`${BACKEND_URL}/api/admin/resumes/upload`, {
      method: 'POST',
      headers: {
    ...
    console.error('❌ Admin resume upload proxy error:', error)
