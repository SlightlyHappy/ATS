  ...
  const fetchActivityLogs = async (isRefresh = false) => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const activityLogs: ActivityLogEntry[] = Array.isArray(responseData) 
          ? responseData 
          : (responseData.logs || responseData.entries || []);
        setActivityLogs(activityLogs);
        
        if (activityLogs.length === 0) {
          toast({
            title: "No Activity Data",
            description: "No activity logs found for the selected criteria.",
            variant: "default",
          });
        }
      } else {
        // Handle API error or no data scenario
        console.warn("Activity logs API returned no data or failed:", response);
        setActivityLogs([]);
        
        // Show user-friendly message
        toast({
          title: "API Error",
          description: response.error || response.message || "Failed to fetch activity logs.",
          variant: "destructive",
        });
      }
    } catch (error) {
