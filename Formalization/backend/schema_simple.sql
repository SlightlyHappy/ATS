-- Simple PostgreSQL schema for Resume Screening Application
...
CREATE INDEX idx_resumes_experience ON resumes(total_experience_years DESC);
CREATE INDEX idx_resumes_reviewed ON resumes(reviewed);
CREATE INDEX idx_resumes_file_hash ON resumes(file_hash);
CREATE INDEX idx_resumes_current_company ON resumes(current_company);
...
CREATE INDEX idx_resumes_search_gin ON resumes USING gin(searchable_content gin_trgm_ops);
