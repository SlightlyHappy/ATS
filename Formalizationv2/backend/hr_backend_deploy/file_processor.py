"""
File Processing System - Lightweight Version
Handles DOCX file processing only (no OCR/PDF support for Railway deployment)
"""
...
# File processing imports (lightweight only)
...
class FileProcessor:
    """Handles file processing for resume analysis - lightweight version"""
    
    def __init__(self):
        """Initialize file processor"""
        self.max_file_size = int(os.getenv('MAX_FILE_SIZE', 10485760))  # 10MB
        self.allowed_extensions = {'.docx'}  # Only DOCX for lightweight deployment
    ...
    def _process_pdf(self, file_content: bytes) -> str:
        """PDF processing disabled for lightweight deployment"""
        logger.warning("PDF processing is disabled in lightweight deployment")
        raise Exception("PDF processing not available in lightweight deployment. Please use DOCX format.")
    ...
    def _process_image(self, file_content: bytes) -> str:
        """Image OCR processing disabled for lightweight deployment"""
        logger.warning("Image OCR processing is disabled in lightweight deployment")
        raise Exception("Image processing not available in lightweight deployment. Please use DOCX format.")
